...
const Create = ({ application, viewOnly }) => {
    ...
    return (<>
        ...
        <div className={`${step === 0 ? "opacity-100 pointer-events-auto" : "opacity-0 !hidden pointer-events-none absolute top-[-1000%]"} w-full mx-auto flex justify-center rounded-lg mt-[60px] mb-10 relative`} style={{
            transition: 'all .15s ease-in'
        }}>
            {/* <Creates/> */}
            <div className={`${!viewOnly && 'md:w-[80%]'} w-full mx-auto p-2 flex flex-col justify-center`}>
                {!viewOnly && <FaTimes size={20} className='absolute -top-1 right-0 cursor-pointer' onClick={() => {
                    setLoading(true)
                    router.back()
                }} />}
                {!viewOnly && <h1 className='md:text-[25px] text-[20px] text-center md:my-5 my-2' >{application ? "Update Application" : 'Create New Application'}</h1>}

                <form method='POST' action='' onSubmit={handleSubmit} >
                    <div className={`flex ${viewOnly ? 'flex-col md:flex-row gap-3 w-full mx-auto' : 'flex-col md:max-w-[700px] mx-auto'}`}>
                        {/* Company Information */}
                    ...
                    <div className={`flex ${viewOnly ? 'flex-col md:flex-row gap-3 w-full mx-auto' : 'flex-col max-w-[700px] mx-auto'}`}>
                        {/* Company Details */}
                        <div className='w-full'>
